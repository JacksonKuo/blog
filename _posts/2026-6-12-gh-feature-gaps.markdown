...
This list will definitely be updated over time.
...
#### Org Secrets
As mentioned in this post, [https://jacksonkuo.github.io/blog/2026/05/27/gh-actions-org-secret-rightsizing.html](https://jacksonkuo.github.io/blog/2026/05/27/gh-actions-org-secret-rightsizing.html), there's no REST API endpoint to switch an existing org secret repo access from `all` to `selected` without re-providing the original secret.

#### Workflows
Workflows have an infinite recursion protection for the default GITHUB_TOKEN.[^5] This protection does not apply to GHApp Tokens. If a workflow using the default GITHUB_TOKEN creates a new PR, any new workflow that spawns from the PR must be approved. However this interferes with Required Workflows and causes them to fail when calls like `peter-evans/create-pull-request` are made.
...
I also wish there was a way to filter GHApp by permissions for both first and third-party without having to call the REST API, alas...
