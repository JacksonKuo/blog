...
def main():
	with open("log.txt", "r") as f:
		...
		# dictionaries are not directly sortable. sorted() takes in iterables, including dict, but will only return the keys in a sorted list. function hash_map.items() turns a dict into a list of tuples, which sent through sorted() and leveraging lambdas will sort the list by the tuple value. 
